# Module 03: Schema Design and Constraints

> Design tables that encode business facts with keys, relationships, constraints, and migrations.
...
Design tables that encode business facts with keys, relationships, constraints, and migrations. This module is intentionally practical: every idea is tied to SQL you can run in `psql` or in any PostgreSQL client.
...
Relational schema design is rooted in E. F. Codd's relational model and later normalization theory. PostgreSQL gives these ideas practical force through constraints that reject invalid states.

```sql
-- Example 1: create or inspect a durable database object for this module.
CREATE TABLE accounts (id bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY, email text NOT NULL UNIQUE);
```
...
CREATE TABLE orders (id bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY, account_id bigint NOT NULL REFERENCES accounts(id));
```
...
ALTER TABLE accounts ADD CONSTRAINT email_has_at CHECK (position('@' in email) > 1);
```
