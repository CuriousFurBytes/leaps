...
> Build the vocabulary, history, setup, and first mental models for AI and machine learning.
...
AI began with symbolic reasoning and expert systems, then expanded as statistical learning and modern deep learning made data-driven systems practical. The key distinction is that traditional software follows explicit instructions, while machine learning fits patterns from examples. This module builds the first mental model: an ML system transforms inputs into outputs using parameters learned from data. The same framing applies whether the system predicts house prices, classifies email, ranks search results, or recommends a next action.
...
- Willingness to run small scripts and inspect outputs.
...
- **Label** — A measurable part of the workflow that shapes what the model can learn. Poor choices here can dominate algorithm choice.
